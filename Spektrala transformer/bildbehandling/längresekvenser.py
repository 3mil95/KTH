...
def diftonger(V1, V2, F0, T, fs, BW):
    X = simplesource(F0)
...
def diftongsynten(V, F, T, fs, BW, P):
    Y = np.array([])
    F = F.astype(float)
    F *= P
    V = V.astype(float)
    ...
    for i in range(len(V)-1):
        F0 = np.linspace(F[i]/fs, F[i+1]/fs, T*fs)
        Y = np.append(Y, diftonger(V[i], V[i+1], F0, T, fs, BW))
    return Y

T = 0.5
V = np.array([vAO, vAO, vI, vE, vUU])
F = np.array([100, 110, 80, 120, 140])
#V = np.array([vAO, vAO, vI, vE, vUU])
#F = np.array([100, 1100, 80, 1200, 140])
P = 1
Y = diftongsynten(V, F, T, fs, BW, P)
